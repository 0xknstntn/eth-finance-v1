...
import "../interface/IOndaV1SwapPoolFactory.sol";
import "../interface/IOndaV1LendingPoolFactory.sol";
import "../interface/IOndaV1AddressProvider.sol";
...
contract OndaV1Router {
	...
	function swapPriorityTokenToToken(address _token0, address _token1, uint _amount) public returns(bool) {
		address factory = IOndaV1AddressProvider(provider).getSwapPoolFactoryAddress();
		address pool = IOndaV1SwapPoolFactory(factory).getSwapPoolAddress(_token0, _token1);
		require(pool != address(0), 'Not exist pool');
		require(_amount > 0, 'Zero amount');
		IOndaV1Pool(pool)._swap(_token0, _token1, address(msg.sender), _amount, 0);
	...
	function addLiquidity(address _token0, address _token1, uint _amount) public returns(bool){
		...
		IOndaV1Pool(pool)._addLiquidity(_token0, _token1, _to, _amount);
	...
	function deleteLiquidity(address _token0, address _token1, uint _amountLiquidity) public returns(bool){
		...
		IOndaV1Pool(pool)._deleteLiquidity(_token0, _token1, address(msg.sender), _amountLiquidity);
	...
	function addLend(address _stablecoin, address _aToken, uint _amount) public returns(bool){
		address factory = IOndaV1AddressProvider(provider).getLendingPoolFactoryAddress();
		address lendingPool = IOndaV1LendingPoolFactory(factory).getLendingPoolAddress(_stablecoin);
		require(lendingPool != address(0), 'Not exist pool');
		require(_amount > 0, 'Zero amount');
		IOndaV1LendingPool(lendingPool)._addLend(_stablecoin, _aToken, address(msg.sender), _amount);
	...
	function deleteLend(address _stablecoin, address _aToken, uint _amount) public returns(bool){
		...
		require(lendingPool != address(0), 'Not exist pool');
		require(_amount > 0, 'Zero amount');
		IOndaV1LendingPool(lendingPool)._deleteLend(_stablecoin, _aToken, address(msg.sender), _amount);
	...
	function newBorrow(address _stablecoin, address _aToken, uint _amount) payable public returns(bool){
		...
		IOndaV1LendingPool(lendingPool)._newBorrow(_stablecoin, _aToken, address(msg.sender), _amount, msg.value);
	...
	function deleteBorrow(address _stablecoin, address _aToken, uint _amount) public returns(bool){
		...
		require(lendingPool != address(0), 'Not exist pool');
		require(_amount > 0, 'Zero amount');
		IOndaV1LendingPool(lendingPool)._deleteBorrow(_stablecoin, _aToken, payable(address(msg.sender)), _amount);
